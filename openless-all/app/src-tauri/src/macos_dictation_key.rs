...
impl TestGuard {
    pub fn begin() -> Result<Self, String> {
        TEST_RUNNING
            ...
            .map_err(|_| "macDictationKeyBusy".to_string())?;
        TEST_CANCELLED.store(false, Ordering::SeqCst);
        Ok(Self)
    }
    pub fn wait(&self) -> Result<(), String> {
        let (tx, rx) = mpsc::channel();
        let monitor = Monitor::start(tx)?;
        let deadline = Instant::now() + Duration::from_secs(30);
        let mut pressed = false;
        while Instant::now() < deadline {
            if TEST_CANCELLED.load(Ordering::SeqCst) {
                return Err("macDictationKeyCancelled".into());
            }
            if !monitor.active() {
                return Err("macDictationKeyUnavailable".into());
            }
            match rx.recv_timeout(Duration::from_millis(100)) {
                Ok(ComboHotkeyEvent::Pressed { .. }) => pressed = true,
                Ok(ComboHotkeyEvent::Released { .. }) if pressed => return Ok(()),
        ...
        Err("macDictationKeyTimeout".into())
    }
}
