...
const oldRecording = { kind: 'voice_state', sessionId: 'old', phase: 'recording', level: 0.6, elapsedMs: 100, seq: 10 } as const;
const newStarting = { kind: 'voice_state', sessionId: 'new', phase: 'starting', level: 0, elapsedMs: 0, seq: 11 } as const;
...
voice = reduceLessComputerVoice(voice, { ...newStarting, phase: 'recording', level: 0.4, elapsedMs: 50, seq: 13 });
voice = reduceLessComputerVoice(voice, newStarting);
...
assertDeepEqual(
  ...
  { seq: 3, kind: 'voice_state', sessionId: '00000000-0000-4000-8000-000000000000', phase: 'recording', level: 0.5, elapsedMs: 120 },
  'React consumes the same serialized Core feedback fixture',
);

const truncated: LessComputerSyncResult = {
  events: [], oldestSequence: 200, latestSequence: 250, truncated: true,
  voiceState: { kind: 'voice_state', sessionId: 'long-transcription', phase: 'transcribing', level: 0, elapsedMs: 4000, seq: 100 },
};
const restored = reconcileLessComputerReplay(0, truncated, []);
let restoredVoice = restored.reset ? null : voice;
if (truncated.voiceState) restoredVoice = reduceLessComputerVoice(restoredVoice, truncated.voiceState, true);
for (const event of restored.events) restoredVoice = reduceLessComputerVoice(restoredVoice, event);
assertDeepEqual(restoredVoice?.phase, 'transcribing', 'an evicted phase snapshot restores a long transcription after reload');
assertDeepEqual(restored.latestAppliedSequence, 250, 'restoring an older projection does not rewind the conversation waterline');
