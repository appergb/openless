...
export const ja: typeof zhCN = {
  ...
  overview: {
    ...
    recentEmptyHint: "まだ音声入力の記録がありません。上の案内に沿って試すと、ここに結果が表示されます。",
    providerHelp: {
      asr: "音声をテキストに変換します。",
    ...
    guide: {
      ...
      servicesDesc: "ここから始めるのがおすすめです。音声認識とテキスト処理のサービスを選びましょう。Omni モードでは、使用するマルチモーダルモデルだけを設定します。",
      permissionsTitle: "ショートカットの状態を確認しましょう",
      permissionsDesc: "ショートカット機能を利用できません。「権限とデータ」で状態と対処方法を確認してください。",
      ...
      permissionsHint: "録音やショートカットが反応しない場合は、「権限とデータ」で権限、マイク、ショートカットの状態を確認してください。",
    },
    kicker: 'DASHBOARD',
    title: '本日の概要',
  ...
  settings: {
    ...
    channels: {
      ...
      validationHint: "実際にリクエストを送信して設定を確認します。サービスの利用枠を消費する場合があります。設定の保存だけでは確認を実行しません。",
      autoSaveHint: "変更は自動保存されます。設定が終わったら、手動で接続を確認できます。",
      nameHint: "同じプロバイダーのチャンネルを区別するための名前です。モデルや接続には影響しません。",
      errModel: "モデル",
      verify: '検証',
      ...
      orderHint: '有効なチャネルのうち、先頭のものを使用します。ドラッグで順序を変更できます。無効なチャネルは末尾に移動します。',
      ...
      hoursAgo: '{{count}}時間前',
      daysAgo: '{{count}}日前',
      localEngineModelHint: '「AI サービスとモデル → ローカルモデル」でモデルをダウンロード・切り替えできます。',
    },
    providers: {
  ...
  startup: {
    ...
    recovery: "再確認してください。解決しない場合はアプリを完全に終了して開き直してください。更新後に発生した場合は、アプリ全体が同じバージョンであることを確認してください。",
    retry: "再確認",
    details: "エラーの詳細",
  },
  modal: {
  ...
  localAsr: {
    ...
    downloadDialogAlreadyHave: "モデルファイルはダウンロード済みです。モデルページで読み込みとテストを行うか、「ASR 音声文字起こし」で対応するプロバイダーを選択してください。",
    downloadDialogDesc: "サイズと説明を確認してモデルをダウンロードします。完了後、「音声認識」で対応するローカルサービスを選択してください。",
    detailRepo: 'リポジトリ',
    hfDownloads: 'ダウンロード数',
    hfLikes: 'いいね',
    hfDescription: 'モデル紹介',
    ...
    libraryEmptyDesc: "音声認識モデルをダウンロードすると、この端末で音声を処理できます。既存のモデルが表示されない場合は、一覧を再読み込みしてください。",
    ...
    detailsTitle: "モデルの詳細",
    testActivateHint: "「読み込みとテスト」はこのモデルを使用中に設定してから、内蔵音声テストを実行します。",
    downloadProgressHint: "開始後はモデルページで進行状況の確認やダウンロードのキャンセルができます。",
    errorDetails: "エラーの詳細",
  },
};
