...
export const en: typeof zhCN = {
  macDictationKey: {
    inactive: 'Interception is not active yet. Retry if it does not start.',
    restore: 'Restore {{shortcut}}',
    label: 'Mac Dictation key',
    use: 'Use the Mac Dictation key',
    description: 'While OpenLess is running, the microphone key uses the recording mode shown above. Ordinary F5 and modifier combinations pass through. Quitting releases the key to macOS.',
    testing: 'Press and release the microphone key once. This test records no audio. {{seconds}} seconds remaining; your previous shortcut is preserved.',
    confirm: 'Key detected; the test has ended. Did macOS Dictation also appear?',
    accept: 'No — use this key',
    keep: 'Yes — keep my previous shortcut',
    active: 'Mac Dictation key interception is active.',
    checking: 'Checking keyboard interception…',
    replace: 'To stop using this key, record another shortcut above or quit OpenLess.',
    retry: 'Retry interception',
    Permission: 'Allow OpenLess in macOS Privacy & Security → Accessibility, then retry. This permission permits keyboard observation; this feature only uses the Dictation key and Fn state and does not store or send typed keys.',
    Timeout: 'No Dictation key was received. Check whether a keyboard remapper is changing it; your previous shortcut is unchanged.',
    Busy: 'Finish the current recording or shortcut test, then retry.',
    Changed: 'Your shortcut changed during setup. The newer setting was preserved; test again to replace it.',
    Unavailable: 'Native interception is unavailable. Your previous shortcut is preserved if activation failed. Check Accessibility permission and retry.',
    permissionSettings: 'Open Accessibility settings'
  },
  app: {
    name: 'OpenLess',
