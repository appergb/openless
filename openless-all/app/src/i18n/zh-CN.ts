...
export const zhCN = {
  ...
  overview: {
    ...
    guide: {
      ...
      servicesDesc: "推荐从这里开始：选择语音识别和文字处理服务；使用 Omni 时，只需配置当前多模态模型。",
  ...
  settings: {
    ...
    channels: {
      ...
      validationHint: "手动发起一次真实请求，检查当前配置；可能消耗服务额度。保存设置不会自动验证。",
  ...
  startup: {
    ...
    recovery: "请重新检查。如果仍然失败，请完全退出后重开应用；升级后出现此问题时，确认已安装完整的同一版本。",
    retry: "重新检查",
    details: "查看错误详情",
  },
  modal: {
  ...
  localAsr: {
    ...
    downloadDialogAlreadyHave: "模型文件已下载。可回到模型页加载并测试，或在「ASR 语音转写」中选择对应供应商。",
    downloadDialogDesc: "查看模型大小与简介，选择后开始下载。下载完成后，在「语音识别」中选择对应的本地服务。",
    detailRepo: '模型仓库',
    hfDownloads: '下载量',
    hfLikes: '收藏数',
    hfDescription: '模型简介',
    ...
    libraryEmptyDesc: "下载一个语音识别模型，让音频在本机处理。已有模型却没有显示时，可重新读取目录。",
    catalogTitle: "模型目录",
