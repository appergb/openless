...
export const ko: typeof zhCN = {
  ...
  overview: {
    ...
    recentEmptyHint: "아직 받아쓰기 기록이 없습니다. 위 안내에 따라 사용해 보면 결과가 여기에 표시됩니다.",
    providerHelp: {
      asr: "음성을 텍스트로 변환합니다.",
    ...
    guide: {
      ...
      servicesDesc: "여기서 시작하는 것을 추천해요. 음성 인식과 텍스트 처리 서비스를 선택하세요. Omni 모드에서는 사용할 멀티모달 모델만 설정하면 됩니다.",
      permissionsTitle: "단축키 상태를 확인하세요",
      permissionsDesc: "현재 단축키 기능을 사용할 수 없습니다. 권한 및 데이터에서 상태와 해결 방법을 확인해 주세요.",
      shortcutsTitle: "녹음 단축키를 설정하세요",
      shortcutsDesc: "편한 단축키를 선택하면 입력 중에 받아쓰기를 시작할 수 있어요.",
      recordingTitle: "녹음 방식을 확인하세요",
      recordingDesc: "서비스 설정이 저장되어 있습니다. 녹음 설정에서 마이크와 원하는 녹음 방식을 선택하세요.",
      tryDictationTitle: "받아쓰기를 해 보세요",
      tryDictationDesc: "입력할 곳에 커서를 놓으세요. {{shortcut}}",
      permissionsHint: "녹음이나 단축키가 반응하지 않나요? 권한 및 데이터에서 권한, 마이크, 단축키 상태를 확인하세요.",
    },
    kicker: 'DASHBOARD',
    title: '오늘 개요',
  ...
  settings: {
    ...
    channels: {
      ...
      validationHint: "실제 요청을 보내 현재 설정을 확인합니다. 서비스 사용량이 차감될 수 있습니다. 설정을 저장해도 자동으로 확인하지 않습니다.",
      autoSaveHint: "변경 사항은 자동으로 저장됩니다. 설정을 마친 후 연결을 직접 확인할 수 있습니다.",
      nameHint: "같은 제공업체의 여러 채널을 구분하는 이름입니다. 모델이나 연결에는 영향을 주지 않습니다.",
      errModel: "모델",
      verify: '검증',
      ...
      orderHint: '사용 중인 채널 중 맨 위의 채널로 요청합니다. 드래그로 순서를 바꾸면 사용하지 않는 채널은 맨 아래로 이동합니다.',
  ...
  startup: {
    ...
    recovery: "다시 확인하세요. 문제가 계속되면 앱을 완전히 종료한 후 다시 여세요. 업그레이드 후 발생한 경우 앱 전체가 동일한 버전인지 확인하세요.",
    retry: "다시 확인",
    details: "오류 세부 정보",
  },
  modal: {
  ...
  localAsr: {
    ...
    downloadDialogAlreadyHave: "모델 파일이 다운로드되었습니다. 모델 페이지에서 불러와 테스트하거나 ASR 음성 전사에서 해당 제공업체를 선택하세요.",
    downloadDialogDesc: "모델 크기와 설명을 확인하고 다운로드하세요. 완료되면 음성 인식에서 해당 로컬 서비스를 선택하세요.",
    detailRepo: '저장소',
    hfDownloads: '다운로드 수',
    hfLikes: '좋아요',
    hfDescription: '모델 소개',
    ...
    libraryEmptyDesc: "음성 인식 모델을 다운로드하면 이 기기에서 오디오를 처리할 수 있습니다. 기존 모델이 보이지 않으면 목록을 새로 불러오세요.",
    ...
    testActivateHint: "불러오기 및 테스트를 실행하면 이 모델을 현재 사용 모델로 설정한 후 내장 오디오 테스트를 진행합니다.",
    downloadProgressHint: "시작 후 모델 페이지에서 진행 상황을 확인하거나 다운로드를 취소할 수 있습니다.",
