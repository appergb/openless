...
export function DownloadProgressBlock({
    ...
    const percent = totalBytes > 0 ? Math.max(0, Math.min(100, downloadedBytes / totalBytes * 100)) : undefined
    const failed = progress?.phase === "failed"
    return (
        <div className="ol-model-download-progress">
            <header><span>{t("localAsr.downloading")}</span><span className={failed ? "is-error" : undefined}>{failed ? t("localAsr.failed") : percent == null ? t("common.loading") : `${Math.round(percent)}%`}</span></header>
            <progress className="ol-model-progress" max={100} value={percent} aria-label={t("localAsr.downloading")} />
            <span>{formatBytes(downloadedBytes)} / {totalBytes > 0 ? formatBytes(totalBytes) : t("localAsr.sizeUnknown")}</span>
            {progress?.file && <small>{progress.file}</small>}
            {failed && progress?.error && <span className="is-error" role="alert">{progress.error}</span>}
...
function ModelStatus({ entry }: { entry: SidebarModelEntry }) {
    ...
    return (
        <span className={`ol-model-status${entry.isActive ? " is-active" : ""}${entry.downloadError ? " is-error" : ""}`}>
...
function ModelChoice({ entry, selected, onSelect }: {
    ...
    return (
        <button type="button" className="ol-model-choice" aria-pressed={selected} onClick={onSelect}>
            ...
            <span className="ol-model-choice-meta">
                ...
                <span>{entry.remoteBytes ? formatBytes(entry.remoteBytes) : t("localAsr.sizeUnknown")}</span>
            </span>
            ...
            {entry.isDownloading && (
                <progress className="ol-model-progress" max={100} value={entry.percent == null ? undefined : Math.max(0, Math.min(100, entry.percent))} aria-label={t("localAsr.downloading")} />
            )}
        </button>
    )
}

export function ModelSidebar({ entries, selectedId, onSelect }: {
    ...
    return (
        <div className="ol-model-library">
            ...
            <div className="ol-model-list" aria-label={t("localAsr.sidebarTitle")}>
                {entries.map(entry => (
                    <ModelChoice key={entry.id} entry={entry} selected={entry.id === selectedId} onSelect={() => onSelect(entry.id)} />
                ))}
            </div>
        </div>
    )
...
function ModelFacts({ entry, fileCount, mirrorLabel }: {
    entry: SidebarModelEntry
    ...
    return (
        <dl className="ol-model-facts">
            <div><dt>{t("localAsr.engineLabel")}</dt><dd>{ENGINE_LABELS[entry.engine]}</dd></div>
            <div><dt>{t("localAsr.sizeLabel")}</dt><dd>{entry.remoteBytes ? formatBytes(entry.remoteBytes) : t("localAsr.sizeUnknown")}</dd></div>
            {fileCount != null && fileCount > 0 && <div><dt>{t("localAsr.files")}</dt><dd>{fileCount}</dd></div>}
            {mirrorLabel && <div><dt>{t("localAsr.mirrorLabel")}</dt><dd>{mirrorLabel}</dd></div>}
            {entry.repo && <div className="ol-model-fact-wide"><dt>{t("localAsr.detailRepo")}</dt><dd>{entry.repo}</dd></div>}
        </dl>
    )
}
...
export function ModelDetailPanel({
    entry, fileCount, mirrorLabel, progress, busy,
    onDownload, onCancel, onDelete, onReveal, onTest, showTest, testResult, testing,
}: {
    ...
    return (
        <div className="ol-model-detail">
            ...
            {entry.downloadError && <div className="ol-model-error" role="alert">{entry.downloadError}</div>}
            {entry.isDownloaded && showTest && <p className="ol-model-muted">{t("localAsr.testActivateHint")}</p>}
            {testResult && <TestResultBlock result={testResult} />}
            <div className="ol-model-actions">
                ...
                {entry.isDownloading && <Btn variant="ghost" onClick={onCancel}>{t("localAsr.cancel")}</Btn>}
                ...
                {entry.isDownloaded && (
                    <>
                        <Btn variant="ghost" disabled={busy || testing || entry.isDownloading} onClick={onReveal}>{t("localAsr.revealDir")}</Btn>
                        <Btn variant="ghost" disabled={busy || testing || entry.isDownloading} onClick={onDelete} style={{ color: "var(--ol-err)" }}>{t("localAsr.delete")}</Btn>
                    </>
                )}
            </div>
        </div>
...
export function DownloadDialog({
    entries, selectedId, onSelect, sizeOf, fileCountOf, hfCardOf,
    busy, loading, error, onRetryCard, onRetryCatalog, onStart, onClose,
}: {
    entries: SidebarModelEntry[]
    selectedId: string | null
    onSelect: (id: string) => void
    ...
    const visibleEntries = engine === "all" ? entries : entries.filter(entry => entry.engine === engine)
    ...
    return createPortal(
        <div className="ol-model-dialog-overlay" onClick={event => {
            if (event.target === event.currentTarget && !busy) onClose()
        }}>
            <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId}
                className={`ol-model-dialog${stackLayout ? " is-stacked" : ""}`} onKeyDown={event => {
                    if (event.key === "Escape") {
                        event.stopPropagation()
                        onClose()
                    }
                    if (event.key !== "Tab") return
                    const focusable = Array.from(event.currentTarget.querySelectorAll<HTMLElement>('button:not(:disabled), [href], input, select, [tabindex="0"]')).filter(element => element.getClientRects().length > 0)
                ...
                <header className="ol-model-dialog-header">
                    ...
                    <button type="button" className="ol-model-close" aria-label={t("common.close")} onClick={onClose}><Icon name="close" size={18} /></button>
                </header>
                    ...
                    {(!stackLayout || !showDetails || !selected) && (
                        <div className="ol-model-catalog">
                            <div className="ol-model-catalog-toolbar">
                                <div className="ol-model-list-heading"><span>{t("localAsr.catalogTitle")}</span><span>{visibleEntries.length}</span></div>
                                {engines.length > 1 && (
                                    <div className="ol-model-filters" role="group" aria-label={t("localAsr.engineLabel")}>
                                        {(["all", ...engines] as const).map(value => (
                                            <button key={value} type="button" aria-pressed={engine === value} onClick={() => {
                                                setEngine(value)
                                                const nextEntries = value === "all" ? entries : entries.filter(entry => entry.engine === value)
                                                if (!nextEntries.some(entry => entry.id === selectedId)) {
                                                    const next = nextEntries.find(entry => !entry.isDownloaded) ?? nextEntries[0]
                                                    if (next) onSelect(next.id)
                                                }
                                            }}>{value === "all" ? t("localAsr.allEngines") : ENGINE_LABELS[value]}</button>
                                        ))}
                                    </div>
                                )}
                            </div>
                            <div className="ol-model-list ol-model-catalog-list">
                                {visibleEntries.map(entry => <ModelChoice key={entry.id} entry={entry} selected={entry.id === selected?.id} onSelect={() => selectModel(entry.id)} />)}
                                {visibleEntries.length === 0 && (
                                    <div className="ol-model-catalog-empty">
                                        <Icon name="download" size={28} />
                                        <p className="ol-model-muted" role="status">{loading ? t("common.loading") : error || t("localAsr.catalogEmpty")}</p>
                                        <Btn disabled={loading} onClick={onRetryCatalog}>{t("localAsr.reloadCatalog")}</Btn>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {selected && (!stackLayout || showDetails) && (
                        <section className="ol-model-catalog-detail">
                            <div className="ol-model-catalog-content">
                                {stackLayout && <button type="button" className="ol-model-back" onClick={showCatalog}>← {t("localAsr.backToCatalog")}</button>}
                                {selected ? (
                                    <>
                                        <div className="ol-model-detail-heading">
                                            <span className="ol-model-eyebrow">{ENGINE_LABELS[selected.engine]}</span>
                                            <h3 ref={detailRef} tabIndex={-1}>{selected.name}</h3>
                                            <ModelStatus entry={selected} />
                                        </div>
                                        <ModelFacts entry={{ ...selected, remoteBytes: sizeOf(selected.id) ?? undefined }} fileCount={fileCountOf(selected.id)} />
                                        {selected.downloadError && <div className="ol-model-error" role="alert">{selected.downloadError}</div>}
                                        {hfCard?.status === "loading" && <p className="ol-model-muted" role="status">{t("common.loading")}</p>}
                                        {hfCard?.status === "error" && (
                                            <div className="ol-model-error">
                                                <p>{t("localAsr.hfCardFailed")}</p>
                                                <details><summary>{t("localAsr.errorDetails")}</summary><p>{hfCard.message}</p></details>
                                                <Btn size="sm" onClick={() => onRetryCard(selected.id)}>{t("common.retry")}</Btn>
                                            </div>
                                        )}
                                        {hfCard?.status === "ok" && (
                                            <div className="ol-model-description">
                                                ...
                                                <dl className="ol-model-facts">
                                                    <div><dt>{t("localAsr.hfDownloads")}</dt><dd>{hfCard.card.downloads.toLocaleString()}</dd></div>
                                                    <div><dt>{t("localAsr.hfLikes")}</dt><dd>{hfCard.card.likes.toLocaleString()}</dd></div>
                                                </dl>
                                            </div>
                                        )}
                                        {selected.isDownloaded && <p className="ol-model-muted">{t("localAsr.downloadDialogAlreadyHave")}</p>}
                                    </>
                                ) : <p className="ol-model-muted">{t("localAsr.detailEmpty")}</p>}
                            </div>
                            <footer className="ol-model-dialog-footer">
                                <span className="ol-model-muted">{selected?.isDownloading ? t("localAsr.downloading") : t("localAsr.downloadProgressHint")}</span>
                                <div className="ol-model-actions">
                                    <Btn variant="ghost" onClick={onClose}>{t("common.close")}</Btn>
                                    <Btn variant="blue" disabled={busy || !selected || selected.isDownloaded || selected.isDownloading} onClick={onStart}>
                                        {selected?.isDownloaded ? t("localAsr.downloadedBadge") : selected?.downloadError ? t("common.retry") : t("localAsr.startDownload")}
                                    </Btn>
                                </div>
                            </footer>
                        </section>
