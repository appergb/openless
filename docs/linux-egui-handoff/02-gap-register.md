...
状态：draft；阶段：partial-implementation；更新：2026-09-07。

初始盘点基线：`2b315572`；本次UI整理基线：`fc38edd1`，更新反映当前工作区。责任依据：[当前范围](../2.0-requirements.md)。以下是源码盘点，不是通过声明。
...
| L09 | 界面部分改善：已增加领域导航、开始/环境引导、独立现有设置和手机输入页、跨页审批/取消；多数 Linux 适用设置、Agent 检测/模型/路径/权限配置仍缺失 | 设置有实际消费者、revision/错误处理；保留现有 Less Computer 输出/审批/取消流程。本次未关闭L09。[领域](./04-ui-domains.md) |
...
## 5. 本轮 UI 进展（2026-09-06）

`L04/L07/L08/L09/L11 / Linux UI / 共享工作区未提交`：现有领域从单页拆为导航分页；开始页与环境准备解释fcitx/plugin、桌面会话、麦克风与Secret Service的实际可知状态。原生未连接时保留诊断步骤，未展示成功。Agent审批固定在页面外，QA/Selection/Agent后台事件保留操作或导航提示，未改变Core接口或扩充原生适配。

本轮只改善入口与显示，不关闭Selection Voice、完整历史/模型管理、Agent高级配置、托盘/更新器及Linux设备/分发缺口。实现、官方来源、定向命令和实际自动证据统一记录在[04本轮记录](./04-ui-domains.md#5-2026-09-06-导航与-linux-准备引导)，不在此重复维护。
